...
class CameraActivity : BaseActivity() {
    ...
    override fun onNewIntent(intent: Intent) {
        ...
        if (permissionWriteStorage != PackageManager.PERMISSION_GRANTED || permissionCamera != PackageManager.PERMISSION_GRANTED) {
            if (permissionWriteStorage != PackageManager.PERMISSION_GRANTED) {
                ActivityCompat.requestPermissions(parent, arrayOf(WRITE_EXTERNAL_STORAGE), REQUEST_EXTERNAL_STORAGE)
            }

            if (permissionCamera != PackageManager.PERMISSION_GRANTED) {
                ActivityCompat.requestPermissions(parent, arrayOf(CAMERA), REQUEST_EXTERNAL_STORAGE)
            }
